...
public class Soulition_programmers_level3_카드짝맞추기 {
	...
	public static void main(String[] args) {
		...
		permutation(n, n, new int[n], temp, 0, 0); // 순열로 카드 순서 
	...
	private static void permutation(int n, int y, int[] perms, int[] input, int depth, int flag) { // 순열로 탐색
		if (depth == y) {
			int[] temp = new int[n];
		...
		for (int i = 0; i < n; i++) 
			if ((flag & (1 << i)) == 0) {
				perms[depth] = input[i];
				permutation(n, y, perms, input, depth + 1, flag | (1 << i));
			}
	}
}